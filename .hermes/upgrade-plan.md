...
- 🔴 **新增：每个模块必须有评估基线。没有指标 = 摸黑走路。ETL 出身不做对账脚本是耻辱。**
...
**准备评估数据集（10 个测试问题 + 标准答案）：**
- 上传 3 个测试文档
- 编写 10 个覆盖不同难度的问题（事实查询 / 推理 / 无答案）
- 人工标注每个问题的"标准答案"和"应在哪些 chunk 里找到"

**评估指标（对标行业 RAGAS 框架）：**

| 指标 | 含义 | 目标值 | ETL 类比 |
|------|------|--------|---------|
| Recall@5 | 5个返回结果命中相关文档的比例 | ≥80% | 数据完整率 |
| Precision@5 | 5个返回结果中真正相关的比例 | ≥60% | 数据准确率 |
| MRR | 第一个相关结果排第几位 | ≥0.7 | 关键数据到达率 |
| Faithfulness | 回答是否基于检索内容（不编造）| ≥90% | 数据可信度 |
| Answer Relevance | 回答与问题的相关度 | ≥80% | 需求匹配度 |

**评估工具：** RAGAS 框架（4 行代码出报告）

**后续优化路线（按基线决定优先级）：**
1. BM25 关键词 + 向量混合检索 → 提升 Recall
2. BGE-Reranker 重排序 → 提升 Precision
3. MMR 多样性 → 避免 5 个结果重复
4. Query 改写 → 处理用户口语化提问
...
**Day 25-评估：LangGraph 质量检查** 🔴 必须
- 路由准确率：10 个混合问题（文档类+数据类+综合），Supervisor 路由正确率 ≥90%
- 多Agent 协作成功率：综合类问题（RAG→SQL→汇总）端到端成功率 ≥80%
- LLM-as-Judge：用 DeepSeek 自动评估回答质量（相关性、准确性、完整性）
...
- 修复所有肉眼可见的 bug
- 截图保存所有功能界面
- **评估：** 端到端响应时间（P50/P95）、UI 操作流畅度、错误场景覆盖率
...
**Day 33-34：测试 + 评估体系** 🔴 必须

**单元测试：**
- RAG 模块（文档加载、向量检索、问答链）
- SQL 模块（SQL 生成、执行、图表）
- LangGraph 工作流集成测试
- API 接口测试（FastAPI TestClient）

**质量评估（对标行业标准）：**
- RAG 检索：RAGAS 框架（Recall@5, Precision@5, MRR, Faithfulness）
- Text-to-SQL：SQL 执行成功率、结果准确性
- LangGraph：路由准确率、端到端成功率
- FastAPI：响应时间 P50/P95、并发测试
